...
- Kubernetes 1.14+
- Helm 2.17.0+
- It's recommended that you have 8 GiB of memory available for this deployment, or at least 4 GiB for the minimum
  requirement. Else, the deployment is expected to fail.
...
- To use pre-rendering:
  ...
  ```yaml
  image:
    repository: <your-namespace>/dhi-opensearch-dashboards
  ```
...
Note: OpenSearch Dashboards requires an OpenSearch cluster. Follow the OpenSearch install instructions to make sure an
OpenSearch cluster exists before proceeding to install this Helm chart.
...
- Pre-rendering:
  ...
  ```console
  helm install my-opensearch-dashboards oci://registry-1.docker.io/<your-namespace>/dhi-opensearch-dashboards-chart --version <version> \
...
Eventually the OpenSearch pods will be up and running.

```bash
$ kubectl get pods
NAME                                                                   READY   STATUS    RESTARTS   AGE
my-opensearch-dashboards-opensearch-dashboards-chart-xxxxxxxxx-xxxxx   1/1     Running   0          30s
```

Finally, to test that OpenSearch Dashboards is indeed up and running. You can open a port-forward rule on port 5601.

```console
kubectl port-forward service/my-opensearch-dashboards-opensearch-dashboards-chart 5601:5601
```

And finally browse to http://localhost:8080 and you will be able to sign into OpenSearch Dashboards with your OpenSearch
