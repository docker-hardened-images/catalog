...
- To use pre-rendering:
  ...
  image:
    repository: <your-namespace>/dhi-opensearch
  persistence:
    image: <your-namespace>/dhi-busybox
  ```
...
Create a Kubernetes secret to hold the OpenSearch admin password. You can change the password to something more secure
than the example below:

```console
kubectl create secret generic opensearch-admin-password \
  --from-literal=adminPassword="Docker123!"
```
...
- Pre-rendering:
  ...
  ```console
  helm install my-opensearch oci://registry-1.docker.io/<your-namespace>/dhi-opensearch-chart --version <version> \
...
my-opensearch-opensearch-0    1/1     Running   0          45s
my-opensearch-opensearch-1    1/1     Running   0          45s
my-opensearch-opensearch-2    1/1     Running   0          45s
```
