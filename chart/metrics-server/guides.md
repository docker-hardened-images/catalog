...
- Kubernetes 1.25+ (recommended 1.31+)
...
- To use pre-rendering:
  ...
  image:
    ...
    repository: <your-namespace>/dhi-metrics-server
  ```

  Replace `registry-1.docker.io` with your own registry if you are using one. Replace `<your-namespace>` with your
  Docker Hub namespace or with your own namespace in your own registry.
...
If you plan to test on Docker Desktop Kubernetes engine you will need to disable TLS verification. You can also defer CA
creation to Helm which is quite simple for validating that everything works. Note that these settings are suitable only
for experiements and development. Never do this on production.

Create a `values-override.yaml` file to set the environment variables for OpenSearch.
...
- Pre-rendering:
  ...
  ```console
  helm install my-metrics-server oci://registry-1.docker.io/<your-namespace>/dhi-metrics-server-chart --version <version> \
...
pod/my-metrics-server-metrics-server-chart-9cb6967bf-m5btd   1/1     Running   0          63s

NAME                                             TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)   AGE
service/kubernetes                               ClusterIP   10.96.0.1       <none>        443/TCP   68d
service/my-metrics-server-metrics-server-chart   ClusterIP   10.105.33.207   <none>        443/TCP   63s

NAME                                                     READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/my-metrics-server-metrics-server-chart   1/1     1            1           63s

NAME                                                               DESIRED   CURRENT   READY   AGE
replicaset.apps/my-metrics-server-metrics-server-chart-9cb6967bf   1         1         1       63s
```
