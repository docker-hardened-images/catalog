...
- To use pre-rendering:
  ...
  ```yaml
  image:
    repository: <your-namespace>/dhi-alertmanager
  configmapReload:
    image:
      repository: <your-namespace>/dhi-prometheus-config-reloader
  ```
...
- Pre-rendering:
  ...
  ```console
  helm install my-alertmanager oci://registry-1.docker.io/<your-namespace>/dhi-alertmanager-chart --version <version> \
...
The StatefulSet's pod should show up and running almost immediately:

```bash
$ kubectl get all
NAME                                       READY   STATUS    RESTARTS   AGE
pod/my-alertmanager-alertmanager-chart-0   1/1     Running   0          73s

NAME                                                  TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)    AGE
service/kubernetes                                    ClusterIP   10.96.0.1       <none>        443/TCP    4d6h
service/my-alertmanager-alertmanager-chart            ClusterIP   10.96.150.211   <none>        9093/TCP   73s
service/my-alertmanager-alertmanager-chart-headless   ClusterIP   None            <none>        9093/TCP   73s

NAME                                                  READY   AGE
statefulset.apps/my-alertmanager-alertmanager-chart   1/1     73s
```

Open a port forward rule:
...
And going to http://localhost:9093 in your web browser should bring AlertManager's web interface.
