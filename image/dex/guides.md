...
This image runs the `dex` binary as an OpenID Connect identity provider. You can invoke it with various command-line
flags and configuration files to set up authentication with different identity providers.
...
Use the `version` command to display version information.

```
$ docker run --rm <your-namespace>/dhi-dex:<tag> dex version
```

## Basic configuration

Dex requires a configuration file to run. The configuration file supports `gomplate` templating, which allows you to
inject values from environment variables. This is particularly useful for managing secrets.

Create a configuration file `config.yaml` using gomplate templating:
...
> [!WARNING]
>
> The example configuration above is not suitable for production use. It is intended for demonstration purposes only. In
> particular, `enablePasswordDB` should be set to `false` when running the fips variant.

Run Dex with environment variables:

```bash
$ docker run --rm \
    -v $(pwd)/config.yaml:/etc/dex/config.yaml:ro \
    -e CLIENT_SECRET=example-client-secret \
    -p 5556:5556 \
    <your-namespace>/dhi-dex:<tag> \
