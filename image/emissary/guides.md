...
This image runs the `emissary`, a Kubernetes-native API gateway built on Envoy Proxy.
...
Use `busyambassador` as the entrypoint and `version` as the command to display version information.

```
$ docker run --rm --entrypoint busyambassador <your-namespace>/emissary:<tag> version
```

Emissary is typically deployed in Kubernetes using manifests or Helm charts. For detailed deployment instructions, and
configuration options, refer to the official documentation at https://emissary-ingress.dev/docs/3.10/
