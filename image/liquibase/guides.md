...
The default entrypoint for the Liquibase image is `liquibase`. You can run the image with the following command to start
a Liquibase container and view the help. At a minimum, replace `<your-namespace>` with your organization's namespace and
...
```
$ docker run --rm <your-namespace>/dhi-liquibase:<tag> --help
```

### Run a migration with a changelog file

To apply a Liquibase changelog to a PostgreSQL database, mount the changelog into the container and provide the database
connection and authentication details. The following command demonstrates how to run a migration using a changelog file
from the host system:

```
$ docker run --rm \
   -v /path/to/changelog.xml:/tmp/changelog.xml \
   <your-namespace>/dhi-liquibase:<tag> \
...
This example assumes you have a PostgreSQL server running at `postgres-server` on port `5432`, the username and password
are set in the environment variables, and that you have a changelog file named `changelog.xml` in the specified path.
Adjust the parameters as necessary for your environment.

### Install packages with lpm (from v5.0.0)

Starting from liquibase 5.0, packages like the database driver are not installed by default. In order to install the
mysql or postgres driver, you can set the `INSTALL_MYSQL` and/or `INSTALL_POSTGRESQL` environment variable to `true`.

Otherwise, you can add drivers with lpm as follows:

```
FROM <your-namespace>/dhi-liquibase:<tag>
