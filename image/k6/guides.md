...
### Load test with a k6 script file
...
export const options = {
  thresholds: {
    http_req_duration: ['p(95)<1000'], // Fail if 95th percentile > 1s
...
For k6 specifically, ensure that:

- Script files are readable by the nonroot user (UID 65532)
- Output directories are writable by the nonroot user
- Configuration files have appropriate permissions
- Volume mounts are accessible to the nonroot user
...
### k6-specific troubleshooting

#### Script file permissions

Ensure script files are readable by the nonroot user:

```bash
# Check file permissions
ls -la script.js  # Should be readable by UID 65532
```

#### Output file permissions

Ensure output directories are writable by the nonroot user:
