...
This hardened image provides Protocol Buffer compiler plugins used to generate Go code from proto definitions. The image
packages three Go protoc plugins that are compatible with Cilium Hubble's proto definitions.
...
| Feature         | Non-hardened Hubble Proto | Docker Hardened Hubble Proto      |
| --------------- | ------------------------- | --------------------------------- |
...
Docker Hardened Images for tooling containers like Hubble Proto do not include a shell or package manager by default.
Since this image only provides static binaries (protoc plugins), debugging typically involves verifying the plugin
versions and testing their functionality.
