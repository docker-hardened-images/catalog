...
# Bundled Java agents live under /datahub/datahub-mae-consumer/lib in DHI
# (upstream stages them at /opt for backward compatibility; we use absolute
# paths to avoid relying on the working directory).
#
# Upstream's EXTRACT_JAR_ENABLED layertools-to-tmpfs optimization is dropped:
# upstream defaults it to false, and enabling it requires a writable /tmp/mae
# extraction dir that the read-only nonroot runtime image does not provide.
JAVA_TOOL_OPTIONS="${JDK_JAVA_OPTIONS:-}${JAVA_OPTS:+ $JAVA_OPTS}${JMX_OPTS:+ $JMX_OPTS}"
if [[ ${ENABLE_OTEL:-false} == true ]]; then
  JAVA_TOOL_OPTIONS="$JAVA_TOOL_OPTIONS -javaagent:/datahub/datahub-mae-consumer/lib/opentelemetry-javaagent.jar"
fi
if [[ ${ENABLE_PROMETHEUS:-false} == true ]]; then
  JAVA_TOOL_OPTIONS="$JAVA_TOOL_OPTIONS -javaagent:/datahub/datahub-mae-consumer/lib/jmx_prometheus_javaagent.jar=4318:/datahub/datahub-mae-consumer/scripts/prometheus-config.yaml"
fi
