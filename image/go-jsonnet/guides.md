...
### Run a different tool by overriding the entrypoint

This image includes the following additional Jsonnet-related tools:

- jsonnetfmt: formats Jsonnet code using standard conventions.
- jsonnet-deps: lists imported dependencies in a Jsonnet file.
- jsonnet-lint: performs static analysis and linting of Jsonnet code.

To run one of the other included tools, override the entrypoint. For example, to run `jsonnetfmt` to format
`test.jsonnet` with a bind mount for input and output, you can use the following command:
