...
```dockerfile
FROM <your-namespace>/dhi-alpine-base:<tag>
...
```bash
$ docker run -it --rm <your-namespace>/dhi-alpine-base:<tag>
```

### Running as the nonroot user

By default, the image runs as root, but it includes a pre-configured nonroot user for security. You can switch to the
nonroot user:
...
Alpine Linux uses the `apk` package manager instead of `apt-get` (Debian/Ubuntu). Some package names may be different
between Alpine and Debian. Common differences:
