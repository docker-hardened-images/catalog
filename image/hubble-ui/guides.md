...
application served by nginx that connects to Hubble Relay to display real-time network flows, service maps, and detailed
flow inspection. It provides rich observability into network traffic, service dependencies, and security policies in
Kubernetes environments.

### Start a Hubble UI image
...
helm upgrade --install cilium cilium/cilium \
  ...
  --set hubble.ui.frontend.image.override="<IMAGE_REPOSITORY>:<IMAGE_TAG>" \
  --set hubble.ui.securityContext.enabled=true \
  --set hubble.ui.securityContext.runAsNonRoot=true \
  --set hubble.ui.securityContext.runAsUser=65532 \
  --set hubble.ui.securityContext.runAsGroup=65532
```
...
| Feature         | Non-hardened Hubble UI  | Docker Hardened Hubble UI                           |
| --------------- | ----------------------- | --------------------------------------------------- |
...
| User            | Runs as nginx user      | Runs as nonroot user (65532)                        |
...
Switching to the hardened Hubble UI image does not require any special changes. You can use it as a drop-in replacement
for the standard Hubble UI (`quay.io/cilium/hubble-ui`) image in your existing workflows and configurations. Note that
