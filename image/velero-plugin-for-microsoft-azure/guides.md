...
## Use the Azure plugin when installing Velero
...
The following example installs Velero with the Azure plugin. It assumes you already have an Azure Storage account with a
container (for example, `velero`) and a resource group.
...
docker run -it --rm \
  ...
  velero install \
    ...
    --backup-location-config resourceGroup=YOUR_RESOURCE_GROUP,storageAccount=YOUR_STORAGE_ACCOUNT \
...
Switching to the hardened Velero Azure plugin image does not require any special changes. You can use it as a drop-in
replacement for the standard `velero/velero-plugin-for-azure` image in your existing workflows and configurations.
