...
stack. `vmselect` performs the incoming queries and fetches the required data from `vmstorage`. \`
...
helm upgrade --install victoria-metrics-cluster victoriametrics/victoria-metrics-cluster \
  ...
  --set vmselect.image.registry="" \
  --set vmselect.image.repository="<IMAGE_REPOSITORY>" \
  --set vmselect.image.tag="<IMAGE_TAG>" \
  --set vmselect.podSecurityContext.enabled=true \
  --set vmselect.podSecurityContext.fsGroup=65532 \
  --set vmselect.securityContext.enabled=true \
  --set vmselect.securityContext.runAsNonRoot=true \
  --set vmselect.securityContext.runAsUser=65532 \
  --set vmselect.securityContext.runAsGroup=65532
```
...
| Feature         | Non-hardened VictoriaMetrics VMSelect | Docker Hardened VictoriaMetrics VMSelect            |
| --------------- | ------------------------------------- | --------------------------------------------------- |
...
Switching to the hardened VictoriaMetrics VMSelect image does not require any special changes. You can use it as a
drop-in replacement for the standard VictoriaMetrics VMSelect (`victoriametrics/vmselect`) image in your existing
