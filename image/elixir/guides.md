...
### What's included in this elixir image

This Docker Hardened elixir image includes:

- `elixir` — runs Elixir scripts and compiled `.beam` files.
...
## Start an elixir image

### Basic usage

The default `cmd` is `iex`:

```bash
$ docker run -it --rm dhi.io/elixir:<tag>-dev
```
...
Mount the host directory and invoke `elixir`:

```bash
$ docker run --rm -v "$(pwd):/scripts" -w /scripts \
  dhi.io/elixir:<tag>-dev \
  elixir hello.exs
```

### Build a production image with Mix releases

Use a multi-stage Dockerfile with the `-dev` variant for building and the runtime variant for the final image. The
`-dev` variant runs as root and includes `mix` and the Elixir compiler. The runtime variant runs as a nonroot user (UID
65532).
...
# Runtime stage
FROM dhi.io/elixir:<tag>
WORKDIR /app
COPY --from=build /build/_build/prod/rel/myapp ./
USER 65532
CMD ["/app/bin/myapp", "start"]
```

For Mix release configuration and project layout, see the
[Mix.Release documentation](https://hexdocs.pm/mix/Mix.Release.html).
