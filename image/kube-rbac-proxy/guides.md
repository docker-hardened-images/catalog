...
This image runs the `kube-rbac-proxy` binary, is a small HTTP proxy for a single upstream, that can perform RBAC
authorization against the Kubernetes AP. It is designed to be deployed within a Kubernetes cluster where it can access
...
Use `kube-rbac-proxy` as the entrypoint and `--version` as the command to display version information.

```
$ docker run --rm --entrypoint kube-rbac-proxy <your-namespace>/dhi-kube-rbac-proxy:<tag> --version
```

kube-rbac-proxy is typically deployed in Kubernetes using manifests or Helm charts. For detailed deployment
instructions, configuration options, and integration with Prometheus and other monitoring systems, refer to the official
documentation at https://github.com/brancz/kube-rbac-proxy
