...
There are many way to deploy and customize KEDA shown in their deploy guides at https://keda.sh/docs/latest/deploy/.

The following is a demonstration of one way to deploy using Docker Hardened Images.
...
```
kubectl patch deployment/keda-metrics-apiserver --patch '{"spec": {"template": {"spec": {"containers": [{"name": "keda-metrics-apiserver", "image": "<your-namespace>/dhi-keda-metrics-apiserver:<tag>"}]}}}}' -n keda
```
