...
object storage (such as MinIO) as a backup storage location for Velero. This image is referenced by Velero at install
time (via `--plugins`) and is not run directly as a long-lived process.

## Use the AWS plugin when installing Velero
...
the standard Velero (`velero/velero-plugin-for-aws`) image in your existing workflows and configurations.
